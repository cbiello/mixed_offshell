module mod_xsects_nnlo_s_ga
  ...
  implicit none
#if(_withchecks == 1)
  real(dp), public, save :: FintNNLO_s_ga(1), FintNNLO_s_ga_onlo(3)
#endif

  public :: xsect_nnlo_s_ga_s12
  public :: xsect_nnlo_s_ga_oewk,xsect_nnlo_s_ga_oqcd
...
contains
  ...
  function xsect_nnlo_s_ga_s12(yRnd,ff,vegasweight)
    use mod_kinematics_lo
    integer :: xsect_nnlo_s_ga_s12
    ...
    if (LOProc%makecut) then
       ...
       call get_respdf_hoppet(xPij,PDFs,aa_lumi,1,1,LOProc,res_lo_1,respdf_1,myPDFs1_Lmu=[xPij_Lmu,xPij_Lmu2])
  ...
  end function xsect_nnlo_s_ga_s12

  function xsect_nnlo_s_ga_oewk(yRnd,ff,vegasweight)
    use mod_kinematics_nlo_z
    implicit none
    integer :: xsect_nnlo_s_ga_oewk
    real(dp15) :: yRnd(30),ff(1),vegasweight
    type(KinConfig) :: HardProc,C1Lim,C2Lim
    !--
    real(dp) :: xx(kNLO_max_full),z
    real(dp) :: kin(3),FintNNLO_s_ga_oewk(3)
    ...
    if (HardProc%makecut) then
       ...
       call get_respdf(ga_lumi,1,1,HardProc,res_ag,respdf)
    ...
    if (C2Lim%makecut) then
       ...
       call get_respdf(ga_lumi,1,1,C2Lim,res_ag,respdf)
    ...
    if (C1Lim%makecut) then
       ...
       call get_respdf(ga_lumi,1,1,C1Lim,res_ag,respdf)
...
#if(_withchecks == 1)
    FintNNLO_s_ga_onlo = FintNNLO_s_ga_oewk
#endif

  end function xsect_nnlo_s_ga_oewk

  function xsect_nnlo_s_ga_oqcd(yRnd,ff,vegasweight)
    use mod_kinematics_nlo_z
    implicit none
    integer :: xsect_nnlo_s_ga_oqcd
    real(dp15) :: yRnd(30),ff(1),vegasweight
    type(KinConfig) :: HardProc,C1Lim
    !--
    real(dp)    :: xx(kNLO_max_full)
    real(dp)    :: FintNNLO_s_ga_oqcd(2),kin(2)
    real(dp)    :: respdf(ipdf),PqgNLO(ipdf),tildePqgNLO(ipdf),sv_logs(ipdf)
    real(dp)    :: res_nlo(2,2),res_nlo_ga(1,1)
    real(dp)    :: z,zz,s15,Pqg0,EC

    xsect_nnlo_s_ga_oqcd = 0
    ...
    call kinematics_nlo_z_is(xx,one,z,HardProc=HardProc,C1Lim=C1Lim,compute_etas=.false.)
    ...
    if (HardProc%makecut.or.HardProc%flag) then
       ...
       kin(1) = zero
       FintNNLO_s_ga_oqcd(1) = zero

    else    

       call res_tree_g_gq(HardProc%AmpMom,res_nlo)

       res_nlo_ga(1,1) = ndn * Qdn2 * sum(res_nlo(:,1)) &
                       + nup * Qup2 * sum(res_nlo(:,2))

       call get_respdf(ga_lumi,1,1,HardProc,res_nlo_ga,respdf)
    ...
    if (C1Lim%makecut.or.C1Lim%flag) then
       ...
       call get_respdf(ga_lumi,1,1,C1Lim,res_nlo_ga,respdf)
       ...
       respdf = tildePqgNLO * respdf * C1Lim%wgt &
              * (2/s15)*(Tr*Pgq_spav(zz)/(one-zz))
       respdf = - respdf

       FintNNLO_s_ga_oqcd(2) = respdf(1)
       kin(2) = respdf(1)
    ...
    call check_ff(ff,xx,FintNNLO_s_ga_oqcd)
    
#if(_withchecks == 1)
    FintNNLO_s_ga_onlo(1:2) = FintNNLO_s_ga_oqcd
#endif

  end function xsect_nnlo_s_ga_oqcd

end module mod_xsects_nnlo_s_ga
