...
contains
  ...
  function xsect_nnlo_s_gq_s12(yRnd,ff,vegasweight)
    ...
    if (LOProc_z%makecut) then
       ...
       gq_int_sub = reshape(calG_QqQl(LOProc_z,z,[Qdn,-Qdn, Qup,-Qup],Q_lep,1),[2,2])
  ...
  function xsect_nnlo_s_gq_oqcd(yRnd,ff,vegasweight)
    ...
    if (HardProc_z%makecut) then
       ...
       call res_tree_g_gq(HardProc_z%AmpMom,res_nlo)
       res_nlo(:,1) = Qdn2 * res_nlo(:,1)
       res_nlo(:,2) = Qup2 * res_nlo(:,2)

       call get_respdf(gq_lumi,1,1,HardProc_z,res_nlo,respdf)
    ...
    if (C1Lim_z%makecut) then
       ...
       call get_respdf(gq_lumi,1,1,C1Lim_z,res_lo,respdf)
  ...
  function xsect_nnlo_s_gq_oewk_is(yRnd,ff,vegasweight)
    ...
    if (C2Lim%makecut) then
       ...
       call get_respdf(gq_lumi,1,1,C2Lim,res_lo,respdf)
    ...
    if (S5Lim%makecut) then
       ...
       call get_respdf(gq_lumi,1,1,S5Lim,res_lo,respdf_bak)
