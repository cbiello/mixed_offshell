program mysummer
  ...
  if (operation.ne.'add' .and. operation.ne.'avg' .and. operation.ne.'nrm' .and. &
       ...
       .and. operation .ne. 'mrg' .and. operation .ne. 'mrr' .and. operation .ne. 'sbt' ) then
     ...
     write(6,*) '       ./mysummer.x mrr nhisto nbins filein file_out'
     stop
  endif

  if (operation.eq.'add') call mysummer_add()
  if (operation.eq.'sbt') call mysummer_sbt()
  ...
  if (operation.eq.'mrr') call mysummer_mrr()
