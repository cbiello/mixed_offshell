...
#
#  -nohash
#   prevents the script from adding hashes in front of text lines (essential
#   when using column name headers in gnuplot)
#
#  -td 
#   works with topdrawer format
#
#  -fc work wiht our histograms [FC]
#  -clean only print numbers and not #, spaces etc
#----------------------------------------------------------------------
#
...
foreach $handle (@filehandles) {
  ...
  while ($line = <$handle>) {
    ...
    if ($active) {
      chomp($line);
      if ($nrun == 0) {
	  if ($line =~ /^\s*[a-z(]/i && $hash) {$lines[$iline] = "# ".$line; $iline++; print "here\n"}
	  else                                 {if (($line =~ /^\s*#/ || $line =~ /^$/ ) && $clean == 1) {} else {$lines[$iline] .=  " ".$line; $iline++;}}
      } else {
	  if (($line =~ /^\s*#/ || $line =~ /^$/ ) && $clean == 1) {} else {$lines[$iline] .=  " ".$line; $iline++;}
      }
	  #$iline++;
