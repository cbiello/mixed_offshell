...
contains
  ...
  subroutine init_user_histo()
    ...
    call new_histo('ptll',0._dp,1000._dp,10._dp)    !-- 4
    call new_histo('ptlm',10._dp, 1000._dp,10._dp)  !-- 5
    call new_histo('ptlp',10._dp, 1000._dp,10._dp)  !-- 6
    call new_histo('ylm',-5._dp, 5._dp,0.1_dp)      !-- 7
    call new_histo('ylp',-5._dp, 5._dp,0.1_dp)      !-- 8
    call new_histo('dRll',0._dp, 7._dp,0.1_dp)      !-- 9
    call new_histo('dYll',-5._dp, 5._dp,0.1_dp)     !-- 10
    call new_histo('dphill',0._dp,1._dp,0.05_dp)    !-- 11
    call new_histo('costhCS',-one,one,0.05_dp)      !-- 12
  ...
  subroutine cut_histo(event)
    ...
    obs(7) = ylm
    obs(8) = ylp
    obs(9) = get_R(rec_mom(:,1),rec_mom(:,2))
    obs(10) = get_dY(rec_mom(:,1),rec_mom(:,2))
    ...
    obs(12) = get_costh_star(rec_mom_lab(:,1),rec_mom_lab(:,2))  !-- TODO: check that it works for massive particles

    return

  end subroutine cut_histo
  
end module mod_cut_histo_dflt
