module mod_aux_sectors_fc
  ...
  public :: qq_lumi,qQp_lumi,qQpb_lumi
...
contains
  ...
  subroutine get_Lmu2(proc,Lmu2,mu2)
    type(KinConfig), intent(in)  :: proc
    real(dp) :: my_spart,my_mu2
    real(dp), intent(out) :: Lmu2(:)
    real(dp), optional, intent(out) :: mu2(:)

    my_spart = proc%Lim_KinInv2(1)
    my_mu2   = proc%muf(1)**2

    if (ipdf == 1) then
       Lmu2 = [log(my_spart/my_mu2)]
       if(present(mu2)) mu2  = [my_mu2]
    elseif (ipdf == 3) then
       Lmu2 = log(my_spart/my_mu2) + [zero,log(four),-log(four)]
       if(present(mu2)) mu2  = my_mu2 * [one,one/four,four]
    endif

  end subroutine get_Lmu2
  ...
  subroutine get_respdf(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
    interface
    ...
    if (ipdf == 1) then
       if (present(factor)) then
          call get_respdf_one(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
       else
          call get_respdf_one(lumi,as_ord,aem_ord,proc,amp2,respdf)
       endif
       elseif (ipdf == 3) then
          if (present(factor)) then
             call get_respdf_three(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
          else
    ...
    contains
      ...
      subroutine get_respdf_one(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
        interface
           function lumi(res,f1,f2)
             use mod_types
             real(dp), intent(in) :: res(:,:),f1(-6:),f2(-6:)
        ...
        if (present(factor)) then
           respdf(1) = pref*lumi(amp2*factor(:,:,1),f1,f2)
        else
      ...
      subroutine get_respdf_three(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
        interface
           function lumi(res,f1,f2)
             use mod_types
             real(dp), intent(in) :: res(:,:),f1(-6:),f2(-6:)
        ...
        if (nohistos) then
           ...
           if (present(factor)) then
              respdf(2) = pref*lumi(amp2*factor(:,:,2),f1,f2)
           else
           ...
           if (present(factor)) then
              respdf(3) = pref*lumi(amp2*factor(:,:,3),f1,f2)
           else
  ...
  subroutine get_respdf_vect(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
    interface
       function lumi(res,f1,f2)
         use mod_types
         real(dp), intent(in) :: res(:,:),f1(-6:),f2(-6:)
    ...
    if (ipdf == 1) then
       if (present(factor)) then
          call get_respdf_vect_one(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
       else
          call get_respdf_vect_one(lumi,as_ord,aem_ord,proc,amp2,respdf)
       endif
    elseif (ipdf == 3) then
       if (present(factor)) then
          call get_respdf_vect_three(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
       else
  ...
  contains
    ...
    subroutine get_respdf_vect_one(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
      interface
         function lumi(res,f1,f2)
           use mod_types
           real(dp), intent(in) :: res(:,:),f1(-6:),f2(-6:)
      ...
      do i = 1,size(amp2,3)
         if (present(factor)) then
            respdf(i,1) = pref*lumi(amp2(:,:,i)*factor(:,:,1),f1,f2)
         else
    ...
    subroutine get_respdf_vect_three(lumi,as_ord,aem_ord,proc,amp2,respdf,factor)
      interface
         function lumi(res,f1,f2)
           use mod_types
           real(dp), intent(in) :: res(:,:),f1(-6:),f2(-6:)
      ...
      if (nohistos) then
         ...
         do i = 1,size(amp2,3)
            if (present(factor)) then
               respdf(i,2) = pref*lumi(amp2(:,:,i)*factor(:,:,2),f1,f2)
            else
         ...
         do i = 1,size(amp2,3)
            if (present(factor)) then
               respdf(i,3) = pref*lumi(amp2(:,:,i)*factor(:,:,3),f1,f2)
            else
